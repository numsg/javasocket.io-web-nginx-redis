...
public class ControllerLogAspectJ {
    ...
    //  前置通知：
    @Before("execution(public * com.gsafety.pivs.notice.controller..*.*(..))")
    ...
    @After("execution(public * com.gsafety.pivs.notice.controller..*.*(..))")
    public void logAfter(JoinPoint joinPoint) {
        logger.info("The method " + joinPoint.getSignature().getName() + " end");
    }

    /*异常通知：仅当连接点抛出异常时执行。*/
    @AfterThrowing(pointcut = "execution(public * com.gsafety.pivs.notice.controller..*.*(..))", throwing = "throwable")
